...
(function () {
  ...
  var CSS = '' +
  '.ps-logbtn{display:inline-flex;align-items:center;gap:8px;background:#D6EAC7;border:2px solid #8EC891;color:#447A1F;font-family:inherit;font-size:13px;font-weight:700;padding:7px 13px;border-radius:8px;cursor:pointer;}' +
  '.ps-logbtn:hover{background:#8EC891;color:#2E5214;}' +
  '.ps-logbtn svg{color:#5DB060;}' +
  '.ps-logbtn:hover svg{color:#2E5214;}' +
  '.ps-log{font-size:12px;color:#463C8F;max-height:240px;overflow:auto;margin-top:8px;}' +
  '.ps-log table{border-collapse:collapse;width:100%;}' +
  '.ps-log th{text-align:left;background:#463C8F;color:#fff;padding:3px 7px;font-size:11px;position:sticky;top:0;}' +
  '.ps-log td{padding:3px 7px;border-bottom:1px solid #EAE5F4;}' +
  ...
  '}' +
  '.ps-gear{background:none;border:none;color:#8C8798;font-size:28px;line-height:1;cursor:pointer;padding:2px 6px;border-radius:8px;transition:color .12s,background .12s,transform .25s;}' +
  '.ps-gear:hover{color:#463C8F;background:#EAE5F4;transform:rotate(60deg);}' +
  ...
  '.ps-modal{background:#fff;color:#463C8F;border:2px solid #463C8F;width:100%;max-width:560px;max-height:88vh;display:flex;flex-direction:column;font-family:"Segoe UI",Arial,Helvetica,sans-serif;}' +
  '.ps-mh{display:flex;justify-content:space-between;align-items:center;background:#463C8F;color:#FF9080;padding:11px 14px;font-size:13px;font-weight:700;letter-spacing:.05em;}' +
  '.ps-mh button{background:none;border:none;color:#EAE5F4;font-size:22px;line-height:1;cursor:pointer;}' +
  '.ps-mb{overflow:auto;padding:14px 16px 20px;}' +
  '.ps-h3{margin:16px 0 8px;font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:#463C8F;border-top:2px solid #EAE5F4;padding-top:12px;}' +
  ...
  '.ps-f input,.ps-f select{padding:9px;border:2px solid #EAE5F4;border-radius:5px;font-family:inherit;font-size:13px;}' +
  '.ps-f input:focus,.ps-f select:focus{outline:none;border-color:#CE0C88;}' +
  '.ps-row{display:flex;gap:6px;}.ps-row input,.ps-row select{flex:1;min-width:0;}' +
  '.ps-mini{background:#EAE5F4;border:none;border-radius:5px;padding:0 11px;font-size:11px;font-weight:700;color:#463C8F;cursor:pointer;font-family:inherit;}' +
  '.ps-mini:hover{background:#E26DB8;color:#fff;}' +
  '.ps-seg{display:inline-flex;border:2px solid #463C8F;border-radius:999px;overflow:hidden;align-self:flex-start;}' +
  '.ps-seg button{font-family:inherit;font-size:13px;font-weight:700;background:#fff;color:#463C8F;border:none;padding:8px 20px;cursor:pointer;}' +
  '.ps-seg button+button{border-left:2px solid #463C8F;}' +
  '.ps-seg button[aria-pressed="true"]{background:#463C8F;color:#fff;}' +
  '.ps-ck{display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:14px;font-weight:700;padding:6px 0;}' +
  '.ps-ck input{width:20px;height:20px;margin-top:1px;accent-color:#463C8F;flex:none;}' +
  ...
  '.ps-bar{height:8px;background:#EAE5F4;border-radius:4px;overflow:hidden;margin:8px 0 4px;display:none;}' +
  '.ps-bar i{display:block;height:100%;width:0;background:#FFC9BF;transition:width .2s;}' +
  '.ps-free{background:#D6EAC7;border-left:4px solid #8EC891;padding:8px 10px;border-radius:0 5px 5px 0;}' +
  ...
  '<div class="ps-mb">' +
    ...
    '<p class="ps-note">Fleet is A330. Every question carries this profile and your longevity as of today.</p>' +
  ...
  var PROFILE = { seat: LS.get('pwa_seat', 'FO'), fleet: 'A330', base: LS.get('pwa_base', 'HNL'),
  ...
  function wireOffline() {
    ...
    navigator.serviceWorker.register('/sw.js').then(function () { return navigator.serviceWorker.ready; })
      .then(function (r) {
        ...
        el('psCore').checked = LS.get('ha330_off_core') === '1';
        el('psAudio').checked = LS.get('ha330_off_audio') === '1';
    ...
    navigator.serviceWorker.addEventListener('message', function (ev) {
      ...
      if (d.type === 'DONE') {
        ...
        if (d.error === 'offline') {
          ...
          LS.set(d.tier === 'audio' ? 'ha330_off_audio' : 'ha330_off_core', '0');
          msg('No connection, so nothing could be saved. Try again when you have signal.');
          return;
        }
        LS.set(d.tier === 'audio' ? 'ha330_off_audio' : 'ha330_off_core', '1');
      ...
      if (d.type === 'CLEARED') {
        busy = false; LS.set(d.tier === 'audio' ? 'ha330_off_audio' : 'ha330_off_core', '0');
        msg((d.tier === 'audio' ? 'Podcast audio' : 'Offline copy') + ' removed from this device.');
        if (d.tier !== 'audio') { el('psAudio').checked = false; el('psAudio').disabled = true; el('psLblAudio').classList.add('off'); LS.set('ha330_off_audio', '0'); }
      }
  ...
  function mount() {
    ...
    (function homeIcon() {
      ...
      var LABEL = /^(?:◀|▶)?\s*(?:A330\s+)?(?:STUDY\s+)?PORTAL(?:\s*(?:◀|▶))?$|^HOME$/i;
    ...
    function flushLog() {
      ...
      var q;
      try { q = JSON.parse(localStorage.getItem('ha330_log_queue') || '[]'); } catch (e) { return; }
      if (!q.length) return;
      fetch(LOG_URL, {
        method: 'POST',
        ...
        try { localStorage.setItem('ha330_log_queue', '[]'); } catch (e) {}
      }).catch(function () {});
    }
    flushLog();
    ...
    function renderLocal() {
      var c = {};
      try { c = JSON.parse(localStorage.getItem('ha330_log_counts') || '{}'); } catch (e) {}
    ...
    function storedKey() { try { return localStorage.getItem('ha330_log_key') || ''; } catch (e) { return ''; } }
    function setKey(v) { try { v ? localStorage.setItem('ha330_log_key', v) : localStorage.removeItem('ha330_log_key'); } catch (e) {} paintKeyRow(); }
  ...
  window.PortalSettings = {
    ...
    profile: function () {
      var L = longevity(PROFILE.doh);
      return { seat: PROFILE.seat, fleet: 'A330', base: PROFILE.base, doh: PROFILE.doh,
