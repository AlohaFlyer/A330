...
PH.append(P('preflight', 'Preflight', 'n', 'PREFLIGHT', 'FCOM PRO-NOR-SOP-03 / SOP-04', [
  ...
  S('PRELIMINARY COCKPIT PREP · CM2 (overhead / pedestal)', TEAL, [
    ...
    box('Batteries check / ELEC', 'Not supplied 6 h or more: BAT 1, BAT 2, APU BAT ... CHECK OFF then VOLTAGE ... CHECK ABOVE 25.5 V · then BAT pb-sw ... AUTO', 'F', False,
    ...
    box('RMP 1 and 2 CHECK ON', 'Green NAV light ... CHECK OFF · COM FREQUENCIES ... TUNE', 'F', False,
    ...
    box('APU FIRE pb-sw', 'APU FIRE pb-sw ... CHECK IN and GUARDED', 'F', False,
        'APU FIRE pb-sw ... CHECK IN and GUARDED CM2', 'PRO-NOR-SOP-04-C-00010902.0001001'),
    box('APU start · bleed', 'APU MASTER SW pb-sw ... ON · APU START pb-sw ... ON · when AVAIL: APU BLEED pb-sw ... ON · X-BLEED ... AUTO · COCKPIT / CABIN TEMP ... AS RQRD', 'F', False,
  ...
  S('PRELIMINARY COCKPIT PREP · CM1 / BOTH', NAVY, [
    fmc('MCDU ON (pre-initialization)', 'Short press of BRT switches ON the MCDU', 'C',
    ...
    box('LOGBOOK and MEL/CDL items * CHECK', 'deferred items list · crosscheck with ECAM recall · MEL / CDL acceptability and influence on the flight plan', 'B', False,
  ...
  S('BEFORE WALKAROUND (PM)', FOREST, [
    box('OXY · HYD · ENG OIL *', 'DOOR SD: OXY ... CHECK PRESSURE · HYD SD: RESERVOIR FLUID LEVEL ... CHECK WITHIN NORMAL RANGE · ENG SD: ENG OIL QUANTITY ... CHECK', 'PM', False,
    ...
    box('Alternate braking check', 'CHOCKS ... CHECK IN PLACE', 'PM', False,
...
PH.append(P('cockpit-prep', 'Cockpit Prep', 'n', 'COCKPIT PREPARATION', 'FCOM PRO-NOR-SOP-06', [
  S('COCKPIT PREPARATION FLOW', NAVY, [
    box('Overhead panel scan', '* ALL WHITE LIGHTS ... OFF', 'PF', False,
    ...
    box('Front instrument panels', '* ISIS ... CHECK (brightness, IAS, altimeter, attitude, no flags) · * NORTH REF ... CHECK (TRUE blue light off)', 'PF', False,
    ...
    box('Center pedestal', 'RMP 1 and 2 ... CHECK ON · Green NAV light ... CHECK OFF · COM FREQUENCIES ... TUNE', 'PF', False,
        'RMP 1 and 2 ... CHECK ON PF Green NAV light ... CHECK OFF PF COM FREQUENCIES ... TUNE PF', 'PRO-NOR-SOP-06-C-00011889.0001001'),
    fmc('FMS preparation · INIT A', 'TO/FROM ... CHECK · ALTN ... ENTER · FLT NBR ... CHECK · COST INDEX ... CHECK · CRZ FL/TEMP ... CHECK · TROPO ... AS RQRD · WINDS ... CHECK', 'PF',
    ...
    box('Glareshield · EFIS control panel', '* BAROMETRIC REFERENCE ... SET/CROSSCHECK · * FD ... CHECK ON · * LS ... AS RQRD · * ND MODE and RANGE ... AS RQRD · * ADF/VOR sw ... AS RQRD', 'B', False,
    ...
    box('PFD and ND * CHECK', 'EFIS DMC selector ... CHECK NORM · PFD/ND not transferred · IAS, FMA, initial target ALT, altimeter, VSI, heading, attitude · ND heading, initial waypoint, VOR/ADF', 'B', False,
    ...
    box('ECAM control panel', '* PRESS pb ... PRESS (CAB PRESS SD shows LDG ELEV AUTO) · * STS pb ... PRESS (INOP SYS compatible with MEL)', 'PF', False,
...
PH.append(P('before-push', 'Before Push', 'n', 'BEFORE PUSHBACK / START CLEARANCE', 'FCOM PRO-NOR-SOP-07', [
  S('LOAD CLOSEOUT · TAKEOFF DATA', PLUM, [
    ...
    box('FINAL TAKEOFF DATA (TPR/TLR) CHECK', '', 'B', False,
        'If the aircraft takes off from an intersection, insert a T.O shift value (even when GPS PRIMARY is active) to increase flight crew awareness of the runway length reduction.',
        'PRO-NOR-SOP-07-A-00021320.0001001'),
...
PH.append(P('before-start', 'Before Start', 'n', 'BEFORE START', 'FCOM PRO-NOR-SOP-07', [
  S('BEFORE START FLOW', PLUM, [
    ...
    box('PARK BRK handle ON (if pushback not required)', 'BRAKES PRESS indicator ... CHECK', 'PF', False,
...
PH.append(P('taxi', 'Taxi', 'n', 'TAXI', 'FCOM PRO-NOR-SOP-10 · SCO', [
  S('DURING TAXI', RED, [
    ...
    box('AFS / flight instruments', 'CLEARED ALTITUDE ON FCU ... CHECK · HDG ON FCU ... IF REQUIRED, PRESET · BOTH FD ... CHECK ON · PFD/ND ... CHECK (both)', 'PM', False,
