...
EDITS={
 ...
 'flows_quiz.html':[
  ...
  ('  svgEl.innerHTML = s;\n}',
   '  svgEl.innerHTML = s;\n  if(grpFilter && flow.groups){ const gc = (flow.groups.find(g=>g.g===grpFilter)||{}).gc; seq.forEach((it,i)=>{ if(it.g!==grpFilter) svgEl.querySelectorAll(\'[data-i="\'+i+\'"]\').forEach(el=>el.style.display="none"); }); const cn=c=>{ const d=document.createElement(\'i\'); d.style.color=c||\'\'; return d.style.color; }; const want=cn(gc); svgEl.querySelectorAll(\'.trace\').forEach(pth=>{ if(cn(pth.style.stroke)!==want) pth.style.display="none"; }); svgEl.querySelectorAll(\'.ghost, .dot.inactive\').forEach(el=>el.style.display="none"); let k=0; seq.forEach((it,i)=>{ if(it.g===grpFilter){ k++; svgEl.querySelectorAll(\'text[data-i="\'+i+\'"]\').forEach(tx=>tx.textContent=String(k)); } }); }\n}'),  # a filtered group counts from 1 (Ryan, 2026-09-22)
  ('  for(let i=0;i<stepIdx;i++){\n    const gh = grpHeader(items, i);',
   '  let gk = 0;\n  for(let i=0;i<stepIdx;i++){\n    if(grpFilter && items[i].g!==grpFilter) continue;\n    gk++;\n    const gh = grpHeader(items, i);'),
  ('s.innerHTML = (i+1) + ". " + (items[i].lt', 's.innerHTML = (grpFilter ? gk : i+1) + ". " + (items[i].lt'),
