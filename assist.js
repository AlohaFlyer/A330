...
(function () {
  ...
  var MAP = {
    'ioe.html': 'ioe', 'flows_quiz.html': 'flows', 'mcdu_preflight.html': 'cdu',
  ...
  var CSS = '' +
  '#as-launch{position:fixed;right:14px;bottom:14px;z-index:9990;display:flex;align-items:center;gap:9px;background:#463C8F;color:#fff;border:2px solid #EE453D;border-radius:999px;padding:9px 16px 9px 10px;font-family:"Segoe UI",Arial,Helvetica,sans-serif;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 3px 14px rgba(1,23,43,.45);}' +
  '#as-launch:hover{background:#831A57;}' +
  '#as-launch img{width:30px;height:30px;border-radius:50%;background:#fff;object-fit:contain;padding:1px;}' +
  '#as-panel{position:fixed;inset:auto 0 0 0;max-height:88vh;z-index:9991;background:#F4F1F9;color:#463C8F;border-top:3px solid #463C8F;display:none;flex-direction:column;font-family:"Segoe UI",Arial,Helvetica,sans-serif;box-shadow:0 -6px 26px rgba(1,23,43,.4);}' +
  '#as-panel.on{display:flex;}' +
  '.as-h{display:flex;justify-content:space-between;align-items:center;gap:10px;background:#463C8F;color:#FF9080;padding:10px 14px;font-size:13px;font-weight:700;letter-spacing:.04em;}' +
  '.as-h button{background:none;border:none;color:#EAE5F4;font-size:22px;line-height:1;cursor:pointer;}' +
  '.as-b{overflow:auto;padding:12px 14px 20px;}' +
  '.as-wrap{max-width:820px;margin:0 auto;}' +
  '#as-q{width:100%;min-height:62px;padding:11px;border:2px solid #463C8F;border-radius:6px;font-family:inherit;font-size:16px;resize:vertical;}' +
  '#as-q:focus{outline:none;border-color:#CE0C88;}' +
  '.as-row{display:flex;align-items:center;gap:14px;margin-top:12px;flex-wrap:wrap;}' +
  '#as-search{flex:1 1 220px;background:#463C8F;color:#fff;border:none;border-radius:6px;padding:15px 18px;font-family:inherit;font-size:16px;font-weight:700;cursor:pointer;}' +
  '#as-search:hover{background:#831A57;}' +
  ...
  '#as-ask .disc{width:76px;height:76px;border-radius:50%;overflow:hidden;background:#fff;border:3px solid #463C8F;display:flex;align-items:center;justify-content:center;}' +
  '#as-ask .disc img{width:100%;height:100%;object-fit:contain;padding:3px;}' +
  '#as-ask .lbl{font-size:11px;font-weight:700;color:#463C8F;}' +
  '.as-modebar{display:flex;align-items:center;gap:9px;margin-top:11px;flex-wrap:wrap;}' +
  '.as-modebar .as-mlbl{font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#6F6B7E;}' +
  '.as-modebar .as-mhelp{font-size:11px;color:#666;flex:1 1 100%;}' +
  '.as-seg{display:inline-flex;border:2px solid #463C8F;border-radius:999px;overflow:hidden;}' +
  '.as-seg button{font-family:inherit;font-size:12px;font-weight:700;letter-spacing:.03em;background:#fff;color:#463C8F;border:none;padding:8px 15px;cursor:pointer;min-height:36px;}' +
  '.as-seg button+button{border-left:2px solid #463C8F;}' +
  '.as-seg button[aria-pressed="true"]{background:#463C8F;color:#fff;}' +
  '.as-chips{display:flex;gap:6px;flex-wrap:wrap;margin-top:9px;font-size:12px;}' +
  '.as-chip{background:#463C8F;color:#fff;border-radius:999px;padding:3px 11px;font-weight:700;letter-spacing:.04em;}' +
  '.as-scope{background:#FFC9BF;color:#463C8F;border-radius:999px;padding:3px 11px;font-weight:700;}' +
  '.as-status{font-size:13px;color:#555;margin-top:12px;min-height:17px;}' +
  '.as-ans{background:#fff;border:2px solid #EE453D;border-left:8px solid #EE453D;padding:14px;margin-top:12px;display:none;}' +
  ...
  '.as-att{position:relative;width:56px;height:56px;border:2px solid #EAE5F4;border-radius:6px;overflow:hidden;background:#fff;}' +
  ...
  '.as-clip{background:#fff;border:2px solid #463C8F;color:#463C8F;border-radius:6px;padding:9px 13px;font-family:inherit;font-size:12.5px;font-weight:700;cursor:pointer;min-height:40px;}' +
  '.as-drop{outline:3px dashed #CE0C88;outline-offset:3px;}' +
  '.as-attnote{font-size:11px;color:#666;margin-top:6px;}' +
  '.as-working{display:flex;align-items:center;gap:9px;font-size:13.5px;color:#463C8F;font-weight:700;}' +
  '.as-working .as-dots{display:inline-flex;gap:4px;}' +
  '.as-working .as-dots i{width:7px;height:7px;border-radius:50%;background:#831A57;display:block;animation:asbl 1.05s infinite ease-in-out;}' +
  ...
  '.as-think summary{cursor:pointer;font-size:11px;font-weight:700;color:#6F6B7E;letter-spacing:.04em;text-transform:uppercase;}' +
  '.as-think .as-tb{white-space:pre-wrap;font-size:12px;line-height:1.55;color:#444;background:#F7F5FB;border-left:4px solid #EAE5F4;padding:8px 10px;margin-top:6px;}' +
  '.as-ans .turn{border-top:2px solid #EAE5F4;margin-top:12px;padding-top:10px;}' +
  '.as-ans .turn:first-child{border-top:none;margin-top:0;padding-top:0;}' +
  '.as-ans .tq{font-size:12.5px;font-weight:700;color:#CE0C88;margin-bottom:6px;}' +
  '.as-fu{border-top:2px solid #EAE5F4;margin-top:12px;padding-top:10px;}' +
  '.as-fu textarea{width:100%;min-height:50px;padding:9px;border:2px solid #EAE5F4;border-radius:6px;font-family:inherit;font-size:16px;resize:vertical;}' +
  '.as-fu textarea:focus{outline:none;border-color:#CE0C88;}' +
  '.as-fubar{display:flex;align-items:center;gap:8px;margin-top:8px;flex-wrap:wrap;}' +
  '.as-fubar button{font-family:inherit;font-size:12.5px;font-weight:700;border-radius:6px;padding:9px 14px;cursor:pointer;min-height:40px;border:2px solid #463C8F;}' +
  '#as-fuask{background:#463C8F;color:#fff;}' +
  '#as-fuask[disabled]{opacity:.5;cursor:not-allowed;}' +
  '#as-funew{background:#fff;color:#463C8F;}' +
  '.as-fubar .as-fucount{font-size:11px;color:#666;}' +
  '.as-res{background:#fff;border:2px solid #EAE5F4;border-left:6px solid #CE0C88;padding:11px 13px;margin-top:9px;}' +
  '.as-res .t{font-size:13px;font-weight:700;}' +
  '.as-res .r{font-size:11px;font-weight:700;color:#CE0C88;margin-top:2px;}' +
  '.as-res .sn{font-size:13.5px;line-height:1.55;color:#333;margin-top:6px;}' +
  '.as-res mark{background:#FFC9BF;padding:0 2px;}';
  ...
  function chips() {
    var p = window.PortalSettings ? window.PortalSettings.profile() : { seat: 'FO', fleet: 'A330', base: 'HNL' };
  ...
  function ask() {
    ...
    load().then(function () {
      ...
      var who = p.seat + ' on the A330 at Hawaiian/Alaska, resident of ' + (p.state || 'an unstated state') +
      ...
      el('as-answ').textContent = window.PortalSettings.modelLabel() + ' · ' + CORPUS.title + ' only · ' +
        p.seat + ' · A330 · ' + p.base + (p.longevity ? ' · ' + p.longevity : '');
      send();
    }).catch(function (e) {
  ...
  function mount() {
    ...
    setMode(lsGet('ha330_assist_mode', 'keywords'), false);
